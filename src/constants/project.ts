import {
  IcAntd,
  IcCSS,
  IcExpress,
  IcHTML,
  IcJavaScript,
  IcMongo,
  IcNext,
  IcNode,
  IcReact,
  IcShadcnUI,
  IcTailwind,
  IcTypeScript
} from './stack';

export const OurProjects = [
  {
    id: 1,
    title: 'Edu Quiz',
    description:
      'Edu Quiz is an online learning platform specifically designed to assist students and teachers in preparing and taking quizzes online. This project was created as part of a final project, which included the development of a frontend for the user interface and a backend for data management and application functionality.',
    image: '/project/project1.png',
    stack: [
      IcJavaScript,
      IcMongo,
      IcExpress,
      IcReact,
      IcNode,
      IcAntd,
      IcTailwind
    ]
  },
  {
    id: 2,
    title: 'Hmc Learn',
    description:
      'HMC Learn is a community project that is being built to help young people in the learning process. Co-created by community team members, the platform uses popular technologies to provide an easy and fun learning experience. Currently, the project is still under construction.',
    image: '/project/project2.png',
    stack: [IcTypeScript, IcNext, IcTailwind, IcShadcnUI]
  },
  {
    id: 3,
    title: 'Tokopedia Clone',
    description:
      'Tokopedia Play Clone is a creative outcome of the Generasi Gigih 3.0 Program Final Project that combines innovative concepts with advanced technology. This project not only explores the use of the MERN stack that has been taught, but also presents an interesting replica of the Tokopedia Play platform.',
    image: '/project/project3.png',
    stack: [IcJavaScript, IcReact, IcTailwind, IcAntd]
  },
  {
    id: 4,
    title: 'Film List',
    description:
      'Film List was created as an innovative platform that delivers movie lists using the open-source TMDB API. Built using the Clean Architecture approach, the platform not only allows users to store a list of their favorite movies.',
    image: '/project/project4.png',
    stack: [IcTypeScript, IcNext, IcTailwind, IcAntd]
  },
  {
    id: 5,
    title: 'Bookshelf App',
    description:
      'Bookshelf App is a website that is used to manage the users reading collection in general and can be accessed by anyone. Currently, the app still uses local storage..',
    image: '/project/project5.png',
    stack: [IcHTML, IcCSS, IcJavaScript]
  }
];
